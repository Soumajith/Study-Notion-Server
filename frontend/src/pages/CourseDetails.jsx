        ...
        <div className='mx-auto box-content px-4 lg:w-[1260px] lg:relative '>
            <div className='mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-center py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]'>
                <div className='relative block max-h-[30rem] lg:hidden'>
                    ...
                    <div className='z-30 my-5 flex flex-col justify-center gap-4 py-5 text-lg text-black'>  
                            <p className='text-4xl font-bold text-black sm:text-[42px]'>{courseDetail?.courseName}</p>
                            <p className='text-richblack-200'>{courseDetail?.courseDescription}</p>
                            <div className='flex gap-x-3 items-center'>
                        <span className='text-yellow-50'>{avgReviewCount || 0}</span>
                        <RatingStars Review_Count={avgReviewCount} />
                        <span className=' md:block hidden md:text-xl text-black'>({courseDetail?.ratingAndReviews?.length} Reviews)</span>
                        {/* student enrolled */}
                        <span className='text-black'>{courseDetail?.studentsEnrolled?.length} students enrolled</span>
                    </div>
                    <div>
                        <p>Created By {courseDetail?.instructor?.firstName}  {courseDetail?.instructor?.lastName}</p>
                    </div>
                    <div className='flex flex-wrap gap-5 text-lg'>
                        <AiOutlineInfoCircle className='text-2xl text-black' />
                        ...
                        <p className='flex items-center gap-2 text-black'><BsGlobe className='text-lgtext-black'/>English</p>
                    </div>
                    </div>
                        ...
                        <p className='space-x-3 pb-4 text-3xl font-semibold text-richblack-5'>
                            ...
                            }
                            {
                                ...
                                (
                                    ...
                                    (<button onClick={()=>{navigate("/dashboard/cart")}} className='blackButton text-black'>Go to Cart</button>) :
                                    (<button onClick={handelAddToCart} className='blackButton text-black'>Add to Cart</button>)
                                )
                            }
                            </>
                            }
                ...
                <div className='right-[1rem] top-[60px] mx-auto hidden min-h-[600px] w-1/3 max-w-[410px] translate-y-24 md:translate-y-0 lg:absolute  lg:block'>
                    <div className='flex flex-col gap-4 rounded-md bg-richblack-5 p-4 text-black'>
                        ...
                        <div className='px-4'>
                            ...
                            <div className='pb-3 pt-6 text-center text-sm text-black'>
            ...
            <div className='mx-auto box-content px-4 text-start text-richblack-5 lg:w-[1260px]'>
                        ...
                        <div className='py-4'>
                            {
                                courseDetail?.courseContent?.map((item, index) => (
                                    <details key={index} className=' border border-solid border-richblack-600 bg-white text-black detailanimatation'>
                                        <summary className='flex cursor-pointer items-start justify-between bg-opacity-20 px-7  py-5 transition-[0.3s]'>
                                            <div className='flex items-center gap-2'>
                                            <FaChevronDown className='arrow '/>
                                            <span className='text-xl text-black'>{item?.sectionName}</span>
                                            </div>
                                        ...
                                        <div className='mt-5'>
                                            {
                                                item?.subSection?.map((subItem, subIndex) => (
                                                    <div key={subIndex} className='relative overflow-hidden bg-richblack-5  p-5 border border-solid border-richblack-600'>
                                                        <div className='flex items-center gap-2'>
                ...
                <div className='flex items-center gap-4 py-4'>
                    ...
                    <p className='text-xl font-semibold text-black'>{courseDetail?.instructor?.firstName} {courseDetail?.instructor?.lastName}</p>
                </div>
                                        ...
                                        <div key={index} className='flex flex-col md:items-baseline gap-3 my-4 mt-12 ga '>
                                            <div className='flex items-center gap-2'>
                                                ...
                                                <div className='flex flex-col'>
                                                    <p className='md:text-xl min-w-max font-semibold text-black'>{item?.user?.firstName} {item?.user?.lastName}</p>
                                                </div>
                                            </div>
