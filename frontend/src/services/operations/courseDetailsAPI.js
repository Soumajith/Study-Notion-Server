...
export const enrollStudentToCourse = async (studentId, courseId, token) => {
  const toastId = toast.loading('Loading...');
  let result = null;
  try {
    const response = await apiConnector(
    ...
    if (!response?.data?.success) {
      throw new Error('Could not enroll student to the course');
    }
...
export const updateStudentGrade = async (courseId, studentId, grade, token) => {
  try {
    const response = await axios.put(
      `http://localhost:4000/api/v1/grade/${studentId}/${courseId}`,
      { grade },
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const getCourseAssignments = async (courseId, studentId, token) => {
  try {
...
export const submitAssignmentSolution = async (assignmentId, studentId, formData, token) => {
  try {
    const response = await axios.post(`http://localhost:4000/api/v1/assignments/${assignmentId}/submit/${studentId}`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const submitCourseFeedback = async (feedbackData, token) => {
  const response = await axios.post(`http://localhost:4000/api/v1/feedback/send`, feedbackData, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const getFeedbackForCourse = async (courseId, token) => {
  try {
      const response = await axios.get(`http://localhost:4000/api/v1/feedback/get/${courseId}`, {
          headers: {
              'Authorization': `Bearer ${token}`
          }
      });
